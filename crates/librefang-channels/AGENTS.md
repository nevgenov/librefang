...
- Per-adapter: `channel-discord`, `channel-slack`, `channel-webhook`, etc. (ntfy, telegram, gotify, mastodon, bluesky, and reddit migrated to sidecars — see `librefang.sidecar.adapters.{ntfy,telegram,gotify,mastodon,bluesky,reddit}` in the SDK.)
